...
static ble_gap_addr_t gap_addr_ = { .addr_type = BLE_GAP_ADDR_TYPE_RANDOM_STATIC };
...
static void init_advertisement_data()
{
    ...
    // requests.
#if PRST_BLE_EXPERIMENTAL_LONG_RANGE
    adv_params_.properties.type = BLE_GAP_ADV_TYPE_EXTENDED_NONCONNECTABLE_NONSCANNABLE_UNDIRECTED;
    adv_params_.primary_phy = BLE_GAP_PHY_CODED;
    adv_params_.secondary_phy = BLE_GAP_PHY_CODED;
#else
    adv_params_.properties.type = BLE_GAP_ADV_TYPE_NONCONNECTABLE_NONSCANNABLE_UNDIRECTED;
#endif
    ...
    uint32_t err_code = sd_ble_gap_adv_set_configure(&adv_handle_, &gap_adv_data_, &adv_params_);
