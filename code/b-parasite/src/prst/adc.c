...
void prst_adc_init()
{
    nrf_saadc_channel_config_t batt_channel_config = NRF_DRV_SAADC_DEFAULT_CHANNEL_CONFIG_SE(PRST_ADC_BATT_INPUT);
    ...
    nrf_saadc_channel_config_t soil_channel_config = NRF_DRV_SAADC_DEFAULT_CHANNEL_CONFIG_SE(PRST_ADC_SOIL_INPUT);
    ...
    nrf_saadc_channel_config_t photo_channel_config = NRF_DRV_SAADC_DEFAULT_CHANNEL_CONFIG_SE(PRST_ADC_PHOTO_INPUT);
...
prst_adc_soil_moisture_t prst_adc_soil_read(double battery_voltage)
{
    ...
    const double percentage = get_soil_moisture_percent(battery_voltage, raw_adc_output);
...
{
    nrf_saadc_value_t raw_photo_output = MAX(0, sample_adc_channel(PRST_ADC_PHOTO_CHANNEL));
    ...
    const float photo_resistance = 1e4f * (battery_voltage - ret.voltage) / ret.voltage;
    ...
    ret.brightness = MAX(0, MIN(mult_value / powf(photo_resistance, pow_value), UINT16_MAX));
